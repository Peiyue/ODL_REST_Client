...
conf=conf()
def add_flow(flow):
	data={"installInHw":"true", "name":'', "node": {"id":"", "type":"OF"}, "ingressPort":"2", "etherType": "0x800","priority":"65530", "actions":["","","","","","","","","",""]}
	...
	#print error_decoder(result.status_code)
        ...
        if result.status_code<400:
                print '[Flow Info]'+ctime()+' Code: '+str(result.status_code)+' '+flow['flowName']+' added successfully '
                               
        else:
                print '[Flow Info]'+ctime()+' Code: '+str(result.status_code)+' '+flow['flowName']+' can not be added '
